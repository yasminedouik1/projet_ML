...
with st.expander("🔬 Hormones & Biologie — Cliquez pour afficher", expanded=False):
    ...
    | **FSH (mIU/mL)** | Hormone Folliculo-Stimulante — stimule la croissance des follicules ovariens. Un taux bas peut indiquer un dysfonctionnement ovarien. | 3.5 – 12.5 mIU/mL |
    ...
    | **RBS (mg/dl)** | Random Blood Sugar — glycémie à un moment aléatoire. Le PCOS est souvent associé à une résistance à l'insuline. | 70 – 140 mg/dl |
    """)

with st.expander("📏 Morphologie & Signes vitaux — Cliquez pour afficher", expanded=False):
    st.markdown("""
    | Champ | Signification | Valeurs normales |
    |-------|--------------|-----------------|
    | **BMI** | Indice de Masse Corporelle (poids/taille²). Un BMI élevé est un facteur de risque du PCOS et de la résistance à l'insuline. | 18.5 – 24.9 (normal) |
    ...
    | **BP Systolique (mmHg)** | Pression artérielle maximale (lors de la contraction du cœur). | 90 – 120 mmHg |
    | **BP Diastolique (mmHg)** | Pression artérielle minimale (entre deux battements). | 60 – 80 mmHg |
    """)
...
with st.expander("🩹 Symptômes cliniques — Cliquez pour afficher", expanded=False):
    ...
    |-------|--------------|----------------|
    | **Cycle menstruel** | Régulier (R) ou Irrégulier (I). Un cycle irrégulier est l'un des critères diagnostiques principaux du PCOS. | ⚠️ Critère majeur |
...
with st.expander("ℹ️ Comment interpréter le résultat ?", expanded=False):
    ...
    > 💡 **Note** : Le modèle a été entraîné sur 2 000 patientes avec un F1-Score de **0.9878** et un ROC-AUC de **0.9970**, 
    ce qui en fait un outil de screening très fiable — mais il ne remplace pas un diagnostic médical.
    """)
